...
async def oauth(request: Request) -> HTTPResponse:
    ...
    if provider:
        ...
        use_after_auth_default_redirect = request.app.config.OAUTH_AFTER_AUTH_DEFAULT_REDIRECT
    client = request.app.oauth_factory(provider=provider)
    if 'code' not in request.args:
        return redirect(client.get_authorize_url(scope=use_scope, redirect_uri=use_redirect_uri))

    token, _data = await client.get_access_token(request.args.get('code'), redirect_uri=use_redirect_uri)
    ...
    return redirect(request['session'].get('after_auth_redirect', use_after_auth_default_redirect))
...
def login_required(async_handler=None, provider=None, add_user_info=True, email_regex=None):
    ...
    async def wrapped(request, **kwargs):
        ...
        if provider:
            try:
                ...
                if provider == "default" and provider_confs:
                    ...
                    raise OAuthConfigurationException("No provider named {} configured".format(provider))
        ...
        user = await fetch_user_info(request, provider, oauth_endpoint_path, email_regex or oauth_email_regex)
...
def setup_providers(  # pylint: disable=too-many-locals
    providers_conf: typing.Dict, oauth_redirect_uri: str, oauth_scope: str, oauth_endpoint_path: str
) -> typing.Dict:
    ...
    for provider_name, provider_conf in providers_conf.items():
        ...
        if module_obj is None:
            raise OAuthConfigurationException(f"Cannot find module {p_module_path} to import OAuth provider")
        p_class = getattr(module_obj, p_class_name, None)
        if p_class is None:
            raise OAuthConfigurationException(f"Cannot find class {p_class_name} in module {p_module_path}")
...
def legacy_oauth_configuration(sanic_app: Sanic, provider_class_link: str, oauth_redirect_uri: str, oauth_scope: str):
    from .core import Client
...
async def create_oauth_factory(sanic_app: Sanic, _loop) -> None:
    ...
    if providers_conf:
        providers = setup_providers(providers_conf, oauth_redirect_uri, oauth_scope, oauth_endpoint_path)
    ...
    def oauth_factory(access_token: str = None, provider=None) -> Client:
        ...
        result = use_provider_class(sanic_app.async_session, access_token=access_token, **use_client_setting)
        return result
