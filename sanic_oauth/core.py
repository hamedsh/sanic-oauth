...
class Client(abc.ABC):
    ...
    @abc.abstractmethod
    async def request(
        self, method: str, url: str, params: Dict[str, str] = None, headers: Dict[str, str] = None, **aio_kwargs
    ) -> ClientResponse:
        pass
    ...
    async def user_info(self, **kwargs) -> Tuple[UserInfo, Dict]:
        ...
        if response.status != 200:
            raise HTTPBadRequest(reason=f'Failed to obtain User information. HTTP status code: {response.status}')
...
class OAuth1Client(Client):  # pylint: disable=abstract-method
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        **params,
        ...
        """Initialize the client."""
        super().__init__(aiohttp_session, base_url, authorize_url, access_token_key, access_token_url, user_info_url)
    ...
    async def request(
        ...
        oparams['oauth_signature'] = self.signature.sign(
            self.consumer_secret, method, url, oauth_token_secret=self.oauth_token_secret, **oparams
        )
        _log.debug("%s %s", url, oparams)
        return await self.aiohttp_session.request(method, url, params=oparams, headers=headers, **aio_kwargs)
    ...
    async def get_access_token(
        ...
        if request_token and self.oauth_token != request_token:
            raise HTTPBadRequest(reason='Failed to obtain OAuth 1.0 access token. Request token is invalid')

        response = await self.request(
            'POST', self.access_token_url, params={'oauth_verifier': oauth_verifier, 'oauth_token': request_token}
        )
        if response.status != 200:
            raise HTTPBadRequest(reason=f'Failed to obtain OAuth 1.0 access token. HTTP status code: {response.status}')
...
class OAuth2Client(Client):  # pylint: disable=abstract-method
    ...
    async def request(
        ...
        return await self.aiohttp_session.request(method, url, params=params, headers=headers, **aio_kwargs)
