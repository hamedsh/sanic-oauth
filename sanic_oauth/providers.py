...
class Bitbucket2Client(OAuth2Client):
    ...
    * API refer: https://confluence.atlassian.com/display/BITBUCKET/Using+the+Bitbucket+REST+APIs
    ...
    async def request(
        ...
        return await self.aiohttp_session.request(method, url, params=params, headers=headers, auth=auth, **aio_kwargs)
...
class TumblrClient(OAuth1Client):
    ...
    * API reference: http://www.tumblr.com/docs/en/api/v2
    ...
    def user_parse(cls, data) -> UserInfo:
        ...
        return UserInfo(id=_user.get('name'), username=_user.get('name'), link=_user.get('blogs', [{}])[0].get('url'))
...
class VimeoClient(OAuth1Client):
    ...
    def user_parse(cls, data) -> UserInfo:
        ...
        return UserInfo(id=_user.get('id'), username=_user.get('username'), first_name=first_name, last_name=last_name)
...
class YahooClient(OAuth1Client):
    ...
    * API reference: https://developer.vimeo.com/apis
    ...
    user_info_url = (
        'https://query.yahooapis.com/v1/yql?q=select%20*%20from%20' 'social.profile%20where%20guid%3Dme%3B&format=json'
    )
...
class PinterestClient(OAuth2Client):
    ...
    * Docs: https://developers.pinterest.com/docs/api/overview/
    ...
    def user_parse(cls, data) -> UserInfo:
        ...
        data = data.get('data', {})
        return UserInfo(
            id=data.get('id'), first_name=data.get('first_name'), last_name=data.get('last_name'), link=data.get('url')
        )
